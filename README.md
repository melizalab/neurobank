...
Resources may be directories or files that act as containers (e.g., ARF files,
https://github.com/melizalab/arf), in which case you are responsible for
assigning identifiers within the container or directory. The `dataset.json` file will contain the mappings from the original names to the new identifiers.
...
The `register` and `deposit` commands both move resource files to the archive. Source files are stored under the `sources` directory in subdirectories with the first two characters of the identifier. For example, if the identifier is `14955422a0fbe3a2bb45111dc91e46e6.wav`, you'll find the file under `sources/14`. Data resources are stored under the `data` directory in subdirectories with the first two characters of their identifiers.
